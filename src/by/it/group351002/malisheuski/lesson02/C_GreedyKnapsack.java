package by.it.group351002.malisheuski.lesson02;
/*
Даны
1) объем рюкзака 4
...
public class C_GreedyKnapsack {
    ...
    double calc(File source) throws FileNotFoundException {
        ...
        for (int i=0;i<4;i++) {
            if (capacity < 4) {
                if (twoDimArray[i][1] <= W - weight) {
                    result = result+twoDimArray[i][0] * twoDimArray[i][1];
                    weight = weight + twoDimArray[i][1];
                    capacity += 1;
                }
                else {
                    result = result+twoDimArray[i][0] * (W - weight);
                    weight = W;
                    capacity += 1;
